...
```
 性质1：
        ...
        如果存在环的话，上述描述的 快指针 fast 和慢指针slow 必然会相遇，且此时slow走的路长
        小于 a + b(可以自行证明)，设其为 a + x；当快慢指针相遇时，快指针已经走了完整的m圈(m >= 1)，有：
        ...
        a = mb - x  -------------  (1)
 性质2：
      当快慢指针相遇时，此时在头节点起一个指针dec，可以证明dec和fast都以1为步长，走a步后就可碰到。
      原因为：
      fast已经走过的距离为 a + mb + x，当再走 a 步之后，fast走过的总距离为 2a + mb + x。带入(1)中，有：
      
      2a + mb + x = a + 2mb -------------  (2)
      
      表明fast正好走了2m个整数圈，到达了环入口处。
```
...
## 合并K个排序链表
[LeetCode中文](https://leetcode-cn.com/problems/merge-k-sorted-lists/submissions/)

合并 k 个排序链表，返回合并后的排序链表。请分析和描述算法的复杂度。

### 解答

利用优先级队列，建立小根堆，那么堆顶是最小的，也就是说重载cmp的时候要return a>b，这样才能保证根节点是最小的。
